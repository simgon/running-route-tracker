  ...
  const handleRouteMigration = async (user: User) => {
    try {
      ...
      if (anonymousId && user.email) {
        // まず認証済みユーザーの既存ルート数を確認（メールアドレスを使用）
        ...
        if (!existingRoutes || existingRoutes.length === 0) {
          console.log(
            "No existing routes found. Migrating routes from anonymous user:",
            anonymousId,
            "to user email:",
            user.email
          );

          const result = await migrateAnonymousRoutesToUser(user.email, anonymousId);
